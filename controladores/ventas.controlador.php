	...
	static public function ctrCrearVenta(){
		...
		if(isset($_POST["nuevaVenta"])){
			...
			if($respuesta == "ok"){
				...
				$printer -> text("Ferreteria Miguel El Foco "."\n");//Nombre de la empresa

				$printer -> text("NIT: 71.759.963-9"."\n");//Nit de la empresa

				$printer -> text("Dirección: Calle Av Padre Hidalgo 224, Sta Ana Pacueco"."\n");//Dirección de la empresa

				$printer -> text("Teléfono: 352 526 1470"."\n");//Teléfono de la empresa
				...
				$printer->text("Gracias por su compra, ¡Vuelva Pronto!"); //Podemos poner también un pie de página
	...
	public function ctrDescargarReporte(){
		...
		if(isset($_GET["reporte"])){
			...
			foreach ($ventas as $row => $item){
				...
				<td style='border:1px solid #eee;'>".(is_array($cliente) && isset($cliente["nombre"]) ? $cliente["nombre"] : '')."</td>
				<td style='border:1px solid #eee;'>".(is_array($vendedor) && isset($vendedor["nombre"]) ? $vendedor["nombre"] : '')."</td>
				...
				if (is_array($listaProductos)) {
					foreach ($listaProductos as $key => $value) {
						if (isset($value["descripcion"])) {
							$objetoXML->writeRaw($value["descripcion"].", ");
						}
					}
				}
